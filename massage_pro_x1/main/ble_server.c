...
#include "device_state.h"  // FIXED: Add this include for device_state_t
#include "esp_log.h"
#include "esp_bt.h"
...
// FIXED: BLE connection state - match the typedef in ble_server.h
// Remove 'static' to match 'extern' declaration in header
...
// Additional handles not in typedef (keep them separate)
...
static void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if,
    ...
    switch (event) {
        ...
        case ESP_GATTS_ADD_CHAR_EVT:
            ...
            if (char_write_handle == 0) {
                char_write_handle = param->add_char.attr_handle;
                ble_state.char_handle = char_write_handle;  // Also store in main state
        ...
        case ESP_GATTS_CONNECT_EVT:
            ...
            memcpy(ble_state.remote_addr, param->connect.remote_bda, 6);
...
esp_err_t ble_server_init(void) {
    ...
    // Set security parameters (no pairing)
...
esp_err_t ble_server_notify(uint8_t *data, uint16_t len) {
    ...
    if (char_notify_handle == 0) {
        ESP_LOGE(TAG, "Notify characteristic not initialized");
        return ESP_ERR_INVALID_STATE;
    }
